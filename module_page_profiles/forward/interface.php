		...
		<div class="profile_user_middle_block">
			<div class="profile_head_user_stats">
				<div class="profile_user_rank_block">
					...
					<img class="profile_head_user_rank" src="<?= $General->arr_general['site'] ?>storage/cache/img/ranks/<?= $Player->found[$Player->server_group]['ranks_pack'] . '/' . $Player->get_rank() ?>.svg" alt="" title="" data-tippy-content="<?= $Translate->get_translate_module_phrase('module_page_profiles', '_Rank'); ?>" data-tippy-placement="top">
				</div>
			...
			<div class="profile_head_user_info">
				<div class="profile_user_badges">
					<div class="role_profile">
						<?php if (empty($Player->get_db_Vips()) && (empty($ASAdmins))) : ?>
						...
						<?php endif; ?>
						<?php if (empty($ASAdmins) != true) : ?>
							<div class="profile_admin_status" data-tippy-placement="top">
								<svg viewBox="0 0 512 512">
						...
						<?php endif; ?>
						<?php 
						$ban_check = $Db->query('IksAdmin', $this->Db->db_data['IksAdmin'][0]['USER_ID'], $this->Db->db_data['IksAdmin'][0]['DB_num'], "SELECT `created`, `sid`, `end`, `time` FROM `iks_bans` WHERE `sid` LIKE '%" . $Player->get_steam_64() . "%' order by `created` desc limit 1");
						!empty($ban_check) && ((empty($ban_check['time']) || $ban_check['end'] >= time())) && (print '<div class="profile_player_banned"><svg viewBox="0 0 512 512"><path d="M367.2 412.5L99.5 144.8C77.1 176.1 64 214.5 64 256c0 106 86 192 192 192c41.5 0 79.9-13.1 111.2-35.5zm45.3-45.3C434.9 335.9 448 297.5 448 256c0-106-86-192-192-192c-41.5 0-79.9 13.1-111.2 35.5L412.5 367.2zM512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256z" /></svg>Забанен</div>');
						?>
					</div>
				</div>
						...
						<div class="profile_user_balance_count">
							<?php if (isset($_SESSION["steamid"]) && ($_SESSION["user_admin"] || ($Player->get_steam_32() == $_SESSION['steamid32']))) : ?>
								<svg viewBox="0 0 22 22">
									<path d="M13.6125 3.6208v3.4833h-1.375V3.6208c0-.2475-.22-.3666-.3666-.3666a.3675.3675 0 0 0-.1375.0274L4.4642 6.0225c-.4859.1833-.7975.6417-.7975 1.1642v.6141a3.4243 3.4243 0 0 0-1.375 2.75V7.1866c0-1.0908.6692-2.0625 1.6866-2.4474l7.2784-2.75a1.811 1.811 0 0 1 .6142-.11c.9166 0 1.7416.7424 1.7416 1.7416ZM19.7083 13.2917v.9167c0 .2475-.1925.4491-.4492.4583h-1.3383c-.4858 0-.9258-.3575-.9625-.8342-.0275-.2841.0825-.55.2658-.7333.165-.1742.3942-.2658.6417-.2658H19.25c.2658.0091.4583.2108.4583.4583Z"></path>
									<path d="M17.8567 11.8708h.935c.5042 0 .9167-.4125.9167-.9167v-.4033c0-1.8975-1.5492-3.4467-3.4467-3.4467H5.7383c-.7791 0-1.4941.2567-2.0716.6967a3.4244 3.4244 0 0 0-1.375 2.75V16.72c0 1.8975 1.5492 3.4466 3.4466 3.4466h10.5234c1.8975 0 3.4467-1.5491 3.4467-3.4466v-.1742c0-.5042-.4125-.9167-.9167-.9167h-.7975c-.88 0-1.7233-.5408-1.9525-1.3933-.1925-.6967.0367-1.3658.495-1.815.3392-.3483.8067-.55 1.32-.55Zm-5.0233-.1833H6.4167c-.3758 0-.6875-.3117-.6875-.6875 0-.3759.3117-.6875.6875-.6875h6.4167c.3758 0 .6875.3116.6875.6875 0 .3758-.3117.6875-.6875.6875Z"></path>
								</svg>
								<?= $Player->get_balance() . ' ' . $Translate->get_translate_module_phrase('module_page_lk_impulse', '_AmountCourse'); ?>
							<?php else : ?>
								Скрыт
							<?php endif; ?>
						</div>
