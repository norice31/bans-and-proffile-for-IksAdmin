	...
	<div class="col-md-12 profile_gap">
		<div class="profile_block_titles">
			<div class="profile_block_content_title col-md-6 center_title">
				...
				<?= $Translate->get_translate_module_phrase('module_page_profiles', '_Bans'); ?> (<?= count($ASBans) ?>)
			</div>
			<div class="profile_block_content_title col-md-6 center_title">
		...
		<div class="profile_block_content">
			<div class="profile_block_about_player max_height fix_gap">
				...
				<?php else : ?>
					...
					<?php for ($i = 0, $c_c = min(sizeof($ASBans), 10); $i < $c_c; $i++) { 
                    $adminname = 0;
                    foreach ($adminlist as $admn) {
                    if ($admn['sid'] == $ASBans[$i]['adminsid'])
                        $adminname = $admn['name'];
                    }
                    ...
                    if (!$adminname) {
                        $adminname = 'Админ';
							...
							?>
							<li class="hover_lbam">
								<span class="bans_comms_none"><?= date("d.m.Y H:i:s", $ASBans[$i]['created']) ?></span>
								<span><?= $ASBans[$i]['reason'] ?></span>
								<span class="bans_comms_none"><a href="<?= $General->arr_general['site'] ?>profiles/<?= $ASBans[$i]['adminsid'] ?>/?search=1"><?= $adminname ?></a></span>
								<span class="bans_comms_none">
									<?php $createdTime = $ASBans[$i]['created']; $duration = $ASBans[$i]['time'];
									if ($duration == 0) {
										echo '<div class="color-red">' . $Translate->get_translate_phrase('_Forever') . '</div>';
									} else { 
                                    if (time() > $createdTime + $duration) { 
                                        echo '<div class="color-dark"><strike>' . $Modules->action_time_exchange($duration) . '</strike></div>'; 
                                        } else { 
                                            echo '<div class="color-blue">' . $Modules->action_time_exchange($duration) . '</div>'; 
                                        } 
                                    } ?>
								</span>
								<span>
					...
					<ul class="bans_comms_list_body bans_comms_list_scroll no-scrollbar">
						<?php for ($i = 0, $c_c = min(sizeof($SBComms), 10); $i < $c_c; $i++) { 
							...
							<li class="hover_lbam">
								...
								<span class="bans_comms_none"><?= date("d.m.Y", $SBComms[$i]['created']) ?></span>
								<span><?= $SBComms[$i]['reason'] ?></span>
								<span class="bans_comms_none"><a href="<?= $General->arr_general['site'] ?>profiles/<?= $SBComms[$i]['adminsid'] ?>/?search=1"><?= $adminname ?></a></span>
								<span>
									<?php
									if ($SBComms[$i]['time'] == '0') {
										echo '<div class="color-red">' . $Translate->get_translate_module_phrase('module_page_profiles', '_Permanent') . '</div>';
									} elseif ($SBComms[$i]['time'] < '0' && time() >= $SBComms[$i]['end']) {
										echo '<div class="color-dark">' . $Translate->get_translate_module_phrase('module_page_profiles', '_Withdrawn') . '</div>';
									} elseif (time() >= $SBComms[$i]['end'] && $SBComms[$i]['time'] != '0') {
										echo '<div class="color-green">' . $Translate->get_translate_module_phrase('module_page_profiles', '_Expired') . '</div>';
									} else
										echo '<div class="color-gray">' . date("d.m.Y", (($SBComms[$i]['created']) + ($SBComms[$i]['time']))) . '</div>';
									?>
								</span>
							</li>
						<?php } ?>
