...
class Player
{
    ...
    public function get_db_ASBans()
    {
        if (!empty($this->Db->db_data['IksAdmin'])) {
                return $this->Db->queryAll('IksAdmin', $this->Db->db_data['IksAdmin'][0]['USER_ID'], $this->Db->db_data['IksAdmin'][0]['DB_num'], "SELECT 
                    `iks_bans`.`adminsid`,
                    `iks_bans`.`sid`,
                    `iks_bans`.`name`,
                    `iks_bans`.`created`,
                    ...
                    `iks_bans`.`reason`
                    FROM `iks_bans`
                    WHERE `sid` = '". $this->get_steam_64() ."' ORDER BY `created` DESC");
        }
    }

    public function get_db_ASComms()
    {
        if (!empty($this->Db->db_data['IksAdmin'])) {
                $MSVoice = $this->Db->queryAll('IksAdmin', $this->Db->db_data['IksAdmin'][0]['USER_ID'], $this->Db->db_data['IksAdmin'][0]['DB_num'], "SELECT `id`, `adminsid`, `sid`, `name`, `created`, `time`, `end`, `reason` FROM `" . $this->Db->db_data['IksAdmin'][0]['Table'] . "mutes` WHERE `sid` LIKE '%" . $this->get_steam_64() . "%' ORDER BY `created` DESC");
                $MSGag = $this->Db->queryAll('IksAdmin', $this->Db->db_data['IksAdmin'][0]['USER_ID'], $this->Db->db_data['IksAdmin'][0]['DB_num'], "SELECT `id`, `adminsid`, `sid`, `name`, `created`, `time`, `end`, `reason` FROM `" . $this->Db->db_data['IksAdmin'][0]['Table'] . "gags` WHERE `sid` LIKE '%" . $this->get_steam_64() . "%' ORDER BY `created` DESC");
                ...
                return array_values($uniqueMSMute);
        }
    }

    public function get_db_ASAdmins()
    {
        if (!empty($this->Db->db_data['IksAdmin'])) {
            return $this->Db->query('IksAdmin', $this->Db->db_data['IksAdmin'][0]['USER_ID'], $this->Db->db_data['IksAdmin'][0]['DB_num'], "SELECT 
            `iks_admins`.`sid`,
            `iks_admins`.`name`,
            `iks_admins`.`flags`,
            `iks_admins`.`immunity`,
